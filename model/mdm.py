...
class MyNetwork(nn.Module):
    def __init__(self):
        ...
        self.fc3 = nn.Linear(160, 192)
        self.fc4 = nn.Linear(192, 224)
        self.fc5 = nn.Linear(224, 256)
...
class MDM(nn.Module):
    def __init__(self, modeltype, njoints, nfeats,
        ...
        if 'style1' in self.cond_mode:
            ...
            if self.n_seed != 0:
                ...
                self.embed_text_body = nn.Linear(451 * n_seed, self.latent_dim)
                self.embed_text_hand = nn.Linear(690 * n_seed, self.latent_dim)
            else:
        ...
        self.latent_z_body_linear = MyNetwork()
        self.latent_z_hand_linear = MyNetwork()
    ...
    def forward(self, x_body, x_hand, timesteps,latent_z_body=None,latent_z_hand=None,hy_flag=False, y=None,uncond_info=False):
        """
        ...
        if hy_flag==False:
            ...
            lz_body = self.latent_z_body_linear(latent_z_body).unsqueeze(0)#bs,256
            lz_hand = self.latent_z_hand_linear(latent_z_hand).unsqueeze(0)#bs,256
            ...
            if 'style1' in self.cond_mode:
                ...
                if self.n_seed != 0:
                    ...
                    emb_1_body = embed_text_body
                    emb_1_hand = embed_text_hand
                else:
            ...
            if 'cross_local_attention' in self.cond_mode:
                if 'cross_local_attention3' in self.cond_mode:
                    ...
                    embed_style_2_body = (torch.cat([emb_1_body + emb_t, lz_body],axis=-1)).repeat(nframes, 1, 1)#512
                    embed_style_2_hand = (torch.cat([emb_1_hand + emb_t, lz_hand],axis=-1)).repeat(nframes, 1, 1)
                    ...
                    xseq_body = torch.cat((emb_1_body + emb_t, lz_body, xseq_body), axis=0)
                    xseq_hand = torch.cat((emb_1_hand + emb_t, lz_hand, xseq_hand), axis=0)
            ...
            if 'cross_local_attention' in self.cond_mode:
                if 'cross_local_attention3' in self.cond_mode:
                    ...
                    xseq_upper_body = xseq_upper_body.permute(1, 0, 2)  # (bs, len, dim)
                    xseq_upper_body = xseq_upper_body.view(bs, nframes + 2, self.num_head, -1)
                    xseq_upper_body = xseq_upper_body.permute(0, 2, 1, 3)  # Need (2, 8, 2048, 64)
                    xseq_upper_body = xseq_upper_body.reshape(bs * self.num_head, nframes + 2, -1)
                    xseq_down_body = xseq_down_body.permute(1, 0, 2)  # (bs, len, dim)
                    xseq_down_body = xseq_down_body.view(bs, nframes + 2, self.num_head, -1)
                    xseq_down_body = xseq_down_body.permute(0, 2, 1, 3)  # Need (2, 8, 2048, 64)
                    xseq_down_body = xseq_down_body.reshape(bs * self.num_head, nframes + 2, -1)

                    pos_emb_upper_body = self.rel_pos_body(xseq_upper_body)
                    pos_emb_down_body = self.rel_pos_body(xseq_down_body)
                    xseq_upper_body, _ = apply_rotary_pos_emb(xseq_upper_body, xseq_upper_body, pos_emb_upper_body)
                    xseq_rpe_upper_body = xseq_upper_body.reshape(bs, self.num_head, nframes + 2, -1)
                    xseq_down_body, _ = apply_rotary_pos_emb(xseq_down_body, xseq_down_body, pos_emb_down_body)
                    xseq_rpe_down_body = xseq_down_body.reshape(bs, self.num_head, nframes + 2, -1)
                    
                    xseq_upper_body = xseq_rpe_upper_body.permute(0, 2, 1, 3)  # [seqlen+1, bs, d]
                    xseq_upper_body = xseq_upper_body.view(bs, nframes + 2, -1)
                    xseq_upper_body = xseq_upper_body.permute(1, 0, 2)

                    xseq_down_body = xseq_rpe_down_body.permute(0, 2, 1, 3)  # [seqlen+1, bs, d]
                    xseq_down_body = xseq_down_body.view(bs, nframes + 2, -1)
                    xseq_down_body = xseq_down_body.permute(1, 0, 2)
